...
function save_to_db(insert_string,config){
    ...
    var schema = config.postgresql.schema ? config.postgresql.schema :  'wim';
    ...
    function save_chunk(data,next){
        ...
        function doit(task, callback) {
            var client = task.client
            var block = task.block
            client.query(insert_string + block.join(',')
                        ,function(err){
                             if(err){
                                 //console.log('all at once failed '+insert_string)
                             }
                             callback(err)
                         })
        }
        ...
        pg.connect(connectionString, function(err, client) {
            ...
            values = _.flatten(values) // grasping at straws
            // first try all at once.  If fail, one by one
            q.push({client:client,block:values}
                  ,function(err){
                       if(err){
                           //return next(err)
                           // break into blocks of 10
                           var j = values.length
                           console.log(j)
                           for (var i = 0; i < j; i+=500){
                               var subblock = values.slice(i,500)
                               q.push({client:client,block:subblock}
                                     ,function(err){
                                          if(err){
                                              //throw new Error('die ' + i)
                                              // one by one
                                              async.forEach(subblock
                                                           ,function(val,once_cb){
                                                                one_at_a_time(client,val,once_cb)
                                                            }
                                                           ,function(err){
                                                                client.resumeDrain()
                                                                //console.log('done with one at a time entry')
                                                            })
                                          }
                                      })
                               // assign a callback
                               q.drain = function(err) {
                                   console.log('all queued records have been processed');
                                   client.resumeDrain()
                                   next(err)
                               }
...
function setup_file_parser (options ){
    if(!options) options = {}
    var _schema = options.schema || schema
    ...
    var speed_collector = make_collector('INSERT INTO '+_schema+'.'+_speed_table+'(site_no,ts,wim_lane_no,veh_speed,veh_count) values ')

    var class_collector = make_collector('INSERT INTO '+_schema+'.'+_class_table+'(site_no,ts,wim_lane_no,veh_class,veh_count) values ')

    var speed_class_collector = make_collector('INSERT INTO '+_schema+'.'+_speed_class_table+'(site_no,ts,wim_lane_no,veh_speed,veh_class,veh_count) values ')
        ...
        stream.on('end',function(err){
            if(err){
                async.series([function(seriescb){
                                  async.parallel([speed_class_collector.done
                                                 ,speed_collector.done
                                                 ,class_collector.done
                                                 ]
                                                ,seriescb)
                              }
                             ,function(paracb){
                                  async.parallel([speed_class_collector.end
                                                 ,speed_collector.end
                                                 ,class_collector.end
                                                 ]
                                                ,paracb)
...
function file_queuer(options,next){
    ...
    return function queuer(file,callback){
        q.push(file,callback)
    }
}
